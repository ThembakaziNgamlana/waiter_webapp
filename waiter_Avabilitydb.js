...
// }
export default function createWaiterAvailabilityDB(db) {
  // Helper function to get the ID of a day
  async function getDayId(day) {
    const dayInfo = await db.one('SELECT id FROM new_selected_days WHERE days = $1', [day]);
    return dayInfo.id;
  }
  ...
  // Combine assignments for the whole week
  async function getAllWaiterAssignments() {
    const assignments = await db.manyOrNone(`
      SELECT waiters.waiter_name, new_selected_days.days
      FROM waiter_schedule
  ...
  return {
    getWaiterAssignmentsForWeek: getAllWaiterAssignments, 
    getAllWaiterAssignments,
    insertWaiter,
    getSelectedDays,
    clearWaiterNames,
